...
proc callback_ps3_game_standart { } {
...
proc makeself {in out original} {
    if {${::SELF} == "lv1.self"} {
	    shell ${::SCETOOL} -0 SELF -1 TRUE -2 00 -3 1ff0000002000001 -4 ff000000 -5 LV1 -6 0004003000000000 -e $in $out
	} elseif {${::SELF} == "lv2_kernel.self"} {
	    shell ${::SCETOOL} -0 SELF -1 TRUE -2 00 -3 1050000003000001 -4 05000002 -5 LV2 -6 0004003000000000 -e $in $out
	} elseif {${::SELF} == "spu_pkg_rvk_verifier.self"} {
	    shell ${::SCETOOL} -0 SELF -1 TRUE -2 01 -3 1070000022000001 -4 07000001 -5 ISO -6 0004003000000000 -e $in $out
	} else {
	    shell ${::MAKESELF} $in $out $original
	}
