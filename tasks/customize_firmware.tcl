...
# Option --customize-game-boot-scelogo: Change default Gameboot scelogo
# Option --customize-game-boot-ps3logo: Change default Gameboot ps3logo
# Option --customize-trophy-sound: Change default Trophy sound
# Option --customize-system-error: Change default system error sound
# Option --customize-system-ok: Change default system ok sound
# Option --customize-cursor-click: Change default cursor click sound
# Option --customize-cursor-back: Change default cursor back sound
...
# Type --customize-game-boot-scelogo: file open {"PNG Image" {png}}
# Type --customize-game-boot-ps3logo: file open {"PNG Image" {png}}
# Type --customize-trophy-sound: file open {"wav Audio" {wav}}
# Type --customize-system-error: file open {"wav Audio" {wav}}
# Type --customize-system-ok: file open {"wav Audio" {wav}}
# Type --customize-cursor-click: file open {"wav Audio" {wav}}
# Type --customize-cursor-back: file open {"wav Audio" {wav}}
...
namespace eval customize_firmware {
    ...
    array set ::customize_firmware::options {
        ...
        --customize-game-boot-scelogo ".png"
        --customize-game-boot-ps3logo ".png"
        --customize-trophy-sound ".wav"
        --customize-system-error ".wav"
        --customize-system-ok ".wav"
        --customize-cursor-click ".wav"
        --customize-cursor-back ".wav"
    ...
    proc main {} {
        ...
        if {[file exists $::customize_firmware::options(--customize-trophy-sound)] || [file exists $::customize_firmware::options(--customize-system-error)] || [file exists $::customize_firmware::options(--customize-system-ok)] || [file exists $::customize_firmware::options(--customize-cursor-click)] || [file exists $::customize_firmware::options(--customize-cursor-back)] } { 
            log "Skipping xmb sounds"
        } else {
            ::modify_rco_file $xmb_sound ::customize_firmware::callback_system_plugin
        }
		...
		if {[file exists $::customize_firmware::options(--customize-firmware-xmb-wave)] == 0 } {
            ...
            ::modify_rco_file $xmb_wave ::customize_firmware::copy_customized_file $::customize_firmware::options(--customize-firmware-xmb-wave)
        }
        
        if {[file exists $::customize_firmware::options(--customize-game-boot-scelogo)] == 0 || [file exists $::customize_firmware::options(--customize-game-boot-ps3logo)] == 0 } {
            log "Skipping Game Boot logos"
        } else {
            ::modify_rco_file $ ::customize_firmware::callback_gameboot
            log "Patching to enable Custom Game Boot logos"
            ::modify_devflash_file  ::customize_firmware::copy_customized_file 
        }
        
        if {[file exists $::customize_firmware::options(--customize-theme-src)] == 0} {
            log "Skipping theme, $::customize_firmware::options(--customize-theme-src) does not exit"
        } else {
	...
	proc set_var_cf { } {
        ...
        set game_boot [file join dev_flash vsh resource custom_render_plugin.rco]
        set scelogo [file join $path scelogo.png]
        set ps3logo [file join $path ps3logo.png]
        set xmb_sound [file join dev_flash vsh resource system_plugin.rco]
        set trophy [file join $path snd_trophy.wav]
        set syser [file join $path snd_system_ng.wav]
        set sysok [file join $path snd_system_ok.wav]
        set cursor [file join $path snd_cursor.wav]
        set cuback [file join $path snd_cancel.wav]
        set decide [file join $path snd_decide.wav]
        set cat_decide [file join $path snd_category_decide.wav]
        set opt [file join $path snd_option.wav]
        set theme [file join dev_flash vsh resource theme 01.p3t]
        set fw_version [file join dev_flash resource sysconf_plugin.rco]
	}
    
    proc callback_gameboot { path args } {   
        if {[file exists $option(--customize-game-boot-scelogo)] == 0 } {
            log "Skipping gameboot scelogo, $::customize_firmware::options(--customize-game-boot-scelogo) does not exist"
        } else {
            log "Replacing Game Boot SCE logo"
            ::customize_firmware::copy_customized_file $scelogo $::customized_firmware::options(--customize-game-boot-scelogo)
        }
      
        if {[file exists $option(--customize-game-boot-ps3logo] == 0 } {
            log "Skipping gameboot ps3logo, $::customize_firmware::options(--customize-game-boot-ps3logo) does not exist"
        } else {
            log "Replacing gameboot ps3logo"
            ::customize_firmware::copy_customized_file $ps3logo $::customized_firmware::options(--customize-game-boot-ps3logo)
        }
    }
    
    proc callback_system_plugin { path args } {   
        if {[file exists $::customize_firmware::options(--customize-trophy-sound)] == 0 } {
            log "Skipping trophy_sound, $::customize_firmware::options(--customize-trophy-sound) does not exist"
        } else {
            ::customize_firmware::copy_customized_file $trophy $::customize_firmware::options(--customize-trophy-sound)
        }
        
        if {[file exists $::customize_firmware::options(--customize-system-error)] == 0 } {
            log "Skipping system_error, $::customize_firmware::options(--customize-system-error) does not exist"
        } else {
            ::customize_firmware::copy_customized_file $syser $::customize_firmware::options(--customize-system-error)
        }      
        
        if {[file exists $::customize_firmware::options(--customize-system-ok)] == 0 } {
            log "Skipping system_ok, $::customize_firmware::options(--customize-system-ok) does not exist"
        } else {
            ::customize_firmware::copy_customized_file $sysok $::customize_firmware::options(--customize-system-ok)
        }
        
        if {[file exists $::customize_firmware::options(--customize-cursor-click)] == 0 } {
            log "Skipping cursor_click, $::customize_firmware::options(--customize-cursor-click) does not exist"
        } else {
            ::customize_firmware::copy_customized_file $cursor $::customize_firmware::options(--customize-cursor-click)
            ::customize_firmware::copy_customized_file $decide $::customize_firmware::options(--customize-cursor-click)
            ::customize_firmware::copy_customized_file $cat_decide $::customize_firmware::options(--customize-cursor-click)
            ::customize_firmware::copy_customized_file $opt $::customize_firmware::options(--customize-cursor-click)
        }
        
        if {[file exists $::customize_firmware::options(--customize-cursor-back)] == 0 } {
            log "Skipping cursor_back, $::customize_firmware::options(--customize-cursor-back) does not exist"
        } else {
            ::customize_firmware::copy_customized_file $cuback $::customize_firmware::options(--customize-cursor-back)
        }  
    }
